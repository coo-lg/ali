            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (property != null) {
                    ...
                    if (valueWrapper != null && valueWrapper.getValue() != null) {
                        mPropertyET.setText(String.valueOf(valueWrapper.getValue()));
                    } else {
                        mPropertyET.setText("");
                    }
                }
...
//        connect();
    ...
    private void report(final String name, final String value, String identifier, ValueWrapper valueWrapper) {
        reportData.clear();
...
//                    aResponse.data =
    ...
    private ITResRequestHandler mCommonHandler = new ITResRequestHandler() {
        ...
        public void onProcess(String identify, Object result, ITResResponseCallback itResResponseCallback) {
            ...
            showToast("收到云端异步服务调用 【" + identify + JSONObject.toJSONString(result) + "】");
            try {
...
//        }
            if (TmpConstant.TYPE_VALUE_INTEGER.equals(property.getDataType().getType())) {
                ...
                if (parseData != DEF_VALUE) {
                    report(property.getName(), value, property.getIdentifier(), new ValueWrapper.IntValueWrapper(parseData));
                } else {
                    showToast("数据格式不对");
                }
                return;
            ...
            if (TmpConstant.TYPE_VALUE_FLOAT.equals(property.getDataType().getType())) {
                ...
                if (parseData != null) {
                    report(property.getName(), value, property.getIdentifier(), new ValueWrapper.DoubleValueWrapper(parseData));
                } else {
                    showToast("数据格式不对");
                }
                return;
            ...
            if (TmpConstant.TYPE_VALUE_BOOLEAN.equals(property.getDataType().getType())) {
                ...
                if (parseData == 0 || parseData == 1) {
                    report(property.getName(), value, property.getIdentifier(), new ValueWrapper.BooleanValueWrapper(parseData));
                } else {
                    showToast("数据格式不对");
                }
                return;
            ...
            if (TmpConstant.TYPE_VALUE_TEXT.equals(property.getDataType().getType())) {
                report(property.getName(), value, property.getIdentifier(), new ValueWrapper.StringValueWrapper(value));
                return;
            }
            if (TmpConstant.TYPE_VALUE_DATE.equals(property.getDataType().getType())) {
                report(property.getName(), value, property.getIdentifier(), new ValueWrapper.DateValueWrapper(value));
                return;
            }
            if(TmpConstant.TYPE_VALUE_ENUM.equalsIgnoreCase(property.getDataType().getType())){
                report(property.getName(), value, property.getIdentifier(),new ValueWrapper.EnumValueWrapper(getInt(value)));
                return;
            }
            if(TmpConstant.TYPE_VALUE_ARRAY.equalsIgnoreCase(property.getDataType().getType())){
                ...
                report(property.getName(), value, property.getIdentifier(),arrayValueWrapper);
                return;
            }
            ...
            if (TmpConstant.TYPE_VALUE_STRUCT.equals(property.getDataType().getType())) {
                try {
                    ...
                    report(property.getName(), value, property.getIdentifier(), new ValueWrapper.StructValueWrapper(dataMap));
                } catch (Exception e){
                    showToast("数据格式不正确");
                }
                return;
